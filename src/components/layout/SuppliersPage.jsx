...
const SuppliersPage = () => {
  ...
  const fetchSuppliers = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch('https://www.emkit.site/fournisseurs/', {
  ...
  const handleDelete = async id => {
    ...
    try {
      const token = localStorage.getItem('accessToken');
      if (!token) { navigate('/login'); return; }
      const res = await fetch(`https://www.emkit.site/fournisseurs/${id}/`, { method: 'DELETE', headers: { 'Authorization': `Bearer ${token}` } });
